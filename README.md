...
1. Point `slides-agent` at a folder containing `.pptx` files
...
```bash
git clone https://github.com/chanukyapekala/slides-agent
cd slides-agent
poetry install
```
...
```bash
slides-agent watch ~/Documents/Presentations
```
...
```bash
slides-agent index ~/Documents/Presentations
```
...
```bash
slides-agent search ~/Documents/Presentations "data pipelines"
```
...
_Auto-generated by slides-agent on 2025-04-04 09:00_
...
```bash
slides-agent watch ~/Documents/Presentations          # macOS/Linux
slides-agent watch "C:\Users\you\Documents\Slides"    # Windows
```
